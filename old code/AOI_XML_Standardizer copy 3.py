...
def update_xml_parameter_names_and_descriptions(xml_content, name_changes, description_changes):
    """Update parameter names and descriptions throughout the entire XML content"""
    ...
    # First, handle description changes using regex to preserve XML structure
    for param_name, new_description in description_changes.items():
        ...
        # Pattern 1: Update CDATA content directly under <Description> for this parameter
        pattern1 = (
            r'(<Parameter[^>]*Name="' + escaped_param_name + r'"[^>]*>.*?'
            r'<Description>\s*)'
            r'(<!\[CDATA\[)(.*?)(\]\]>)'
        ...
        # Apply the replacement for main description
        ...
        # Pattern 2: Update CDATA content inside <LocalizedDescription Lang="en-US"> for this parameter
        pattern2 = (
            r'(<Parameter[^>]*Name="' + escaped_param_name + r'"[^>]*>.*?'
        ...
        # Apply the replacement for localized description
    ...
    # Then, handle name changes using regex for exact matches
...
if uploaded_file:
    ...
    with tabs[0]:  # Parameters Editor Tab
        if param_data:
            ...
            # Create editable dataframe - include Description column as editable
            ...
            edited_df = st.data_editor(
                display_df,
                disabled=["🚨", "Name", "Usage", "DataType", "TagType", "FailureReasons"],  # Make these read-only, but Description is now editable
                column_config={
            ...
            if total_changes_count > 0:
                ...
                if desc_changes_count > 0:
                    ...
                    # Add original description for comparison
                ...
                st.session_state.description_changes = description_mapping
                st.session_state.edited_df = edited_df
                
            else:
                st.info("No changes proposed.")
                st.session_state.name_changes = {}
                st.session_state.description_changes = {}
    ...
    with tabs[2]:  # Download Tab
        ...
        has_name_changes = 'name_changes' in st.session_state and st.session_state.name_changes
        has_desc_changes = 'description_changes' in st.session_state and st.session_state.description_changes
        
        if has_name_changes or has_desc_changes:
            st.markdown("**Proposed Changes:**")
            ...
            if has_desc_changes:
                ...
                desc_changes_df = pd.DataFrame([
                    ...
                    for name, desc in st.session_state.description_changes.items()
                ])
                st.dataframe(desc_changes_df, use_container_width=True)
            ...
            if st.button("Apply Changes", key="apply_changes"):
                try:
                    ...
                    name_changes = st.session_state.get('name_changes', {})
                    description_changes = st.session_state.get('description_changes', {})
                    
                    # Apply the changes
                    updated_xml, changes_made, change_details = update_xml_parameter_names_and_descriptions(
                    ...
                    st.success(f"Successfully updated {changes_made} parameter references and descriptions throughout the XML file!")
                    
                    # Show detailed breakdown of changes
                    if change_details:
                        st.markdown("**Change Details:**")
                        
                        # Separate name and description changes
                        name_details = [d for d in change_details if d.get("type") == "name"]
                        desc_details = [d for d in change_details if d.get("type") == "description"]
                        
                        if name_details:
                            st.markdown("##### Name Changes:")
                            name_details_df = pd.DataFrame(name_details)
                        ...
                        if desc_details:
                            st.markdown("##### Description Updates:")
                            desc_details_df = pd.DataFrame(desc_details)
                        ...
                        if name_details:
                            st.info("💡 Name changes include parameter definitions, ladder logic references, HMI bindings, and any other occurrences throughout the file.")
